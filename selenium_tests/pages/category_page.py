...
class CategoryPage:
    ...
    EDIT_AMOUNT_INPUT_CLASS = (By.CLASS_NAME, "edit-amount-input")
    EDIT_SUBMIT_CLASS = (By.CLASS_NAME, "confirm-edit-category")
    DELETE_CONFIRM_BTN = (By.CLASS_NAME, "confirm-delete-category")
    ...
    def add_category(self, category_text="Alimentación", amount="100"):
        ...
        select_element = self.wait.until(
            EC.visibility_of_element_located(self.ADD_SELECT_CATEGORY)
        )
        
        Select(select_element).select_by_visible_text(category_text)
        
        self.wait.until(
            EC.visibility_of_element_located(self.ADD_AMOUNT_INPUT)
        ).send_keys(amount)
        
        self.wait.until(
            EC.element_to_be_clickable(self.ADD_SUBMIT)
        ).click()
    ...
    def edit_category(self, category_name, new_amount="150"):
        ...
        edit_button = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, xpath_edit_btn))
        )
        edit_button.click()

        edit_input = self.wait.until(
            EC.visibility_of_element_located(self.EDIT_AMOUNT_INPUT_CLASS)
        )
        edit_input.clear()
        edit_input.send_keys(new_amount)

        save_btn = self.wait.until(
            EC.element_to_be_clickable(self.EDIT_SUBMIT_CLASS)
        )
        save_btn.click()
    ...
    def delete_category(self, category_name):
        ...
        delete_button = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, xpath_delete_btn))
        )
        delete_button.click()
    ...
    def get_category_amount(self, category_name):
        ...
        try:
            amount_element = category_card.find_element(
                By.CLASS_NAME, "category-amount"
            ).text
            budget_part = amount_element.split("/")[1].strip()
            return budget_part.replace("€", "").strip()
        except:
            over_budget_element = category_card.find_element(
                By.CLASS_NAME, "over-budget"
            ).text
    ...
    def is_category_over_budget(self, category_name):
        ...
        elements = category_card.find_elements(
            By.CLASS_NAME, "over-budget"
        )
